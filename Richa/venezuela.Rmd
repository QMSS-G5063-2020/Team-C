...
```{r, message=FALSE}
# Joining the destination data of 2018 with the shape file

# Changing the USA (INS/DHS) to United States and United Kingdom of Great Britain and Northern Ireland to United Kingdom to match shape file names

#destination_data_ven_2018$destination[destination_data_ven_2018$destination == "USA (INS/DHS)"] <- "United States"
#destination_data_ven_2018$destination[destination_data_ven_2018$destination == "United Kingdom of Great Britain and Northern Ireland"] <- "United Kingdom"

# World_ven <- merge(World, destination_data_ven_2019, by.x = "sovereignt", by.y = "country", all.x = TRUE)
# World_ven$is_ven <- ifelse(World_ven$sovereignt == "Venezuela", 1, 0)
# 
# tmap_mode("view")
# data(World_ven)
# tm_shape(World_ven) +
#   tm_polygons("is_ven") + 
#   tm_bubbles(size = "value", col = "value", alpha = 0.5) +
#   tm_legend(show = FALSE) +
#   tm_view(alpha = 1, set.zoom.limits = c(3,5), basemaps = "Stamen.Watercolor")


## world <- read.csv("Data/country_centroids_az8.csv")
...
leaflet::leaflet() %>%
  ...
  addCircles(data = world_ven,
             lat = world_ven@data$Latitude,
             lng = world_ven@data$Longitude,
             radius = world_ven@data$value,
             fillColor = world_ven@data$value,
             opacity = 0.8, fillOpacity = 0.7,
             popup = content, label = content, labelOptions(
                    style = list("font-weight" = "normal", padding = "3px 8px"),
                    textsize = "15px",
                    direction = "auto")) #%>%
  #addMarkers(lat = -66.1818412311, lng = 7.12422421273, popup = "Venezuela", label = "Venezuela")
